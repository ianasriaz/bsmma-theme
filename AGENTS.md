...
```
.
├── assets          # Stores static assets (including our compiled tailwind.css)
├── blocks          # Reusable, nestable, customizable components
├── config          # Global theme settings and customization options
├── css             # Tailwind CSS source files (e.g. tailwind-source.css)
...
- This directory includes our compiled `tailwind.css` generated by the Tailwind CLI
- Avoid using `{% stylesheet %}` in favor of Tailwind CSS utility classes. Only put static files necessary for every page here
...
### CSS & JavaScript & Design System

- **Tailwind CSS v4 & Design System**: Use Tailwind utility classes directly in Liquid/HTML files. We have a custom design system configured in `css/tailwind-source.css`.
- **Colors**: Always prefer semantic tokens like `text-accent`, `bg-surface`, `text-text-main`, `border-border`, and `bg-background` instead of arbitrary colors.
- **Typography**: Use `font-sans` for body text and `font-display` for headings.
- Avoid writing custom CSS with the `{% stylesheet %}` tag unless absolutely necessary for things Tailwind cannot handle.
- Write JavaScript per component using the `{% javascript %}` tags (only supported in `snippets/`, `blocks/`, and `sections/`).
...
When defining the `{% schema %}` tag on sections and blocks, map them to Tailwind CSS utility classes and inline styles instead of traditional CSS classes. Use our design system tokens wherever possible.

**Single property settings**: For settings that correspond to a dynamic numeric value, map them to inline styles that Tailwind can inherit, or use Tailwind's arbitrary values if applicable:
```liquid
...
**Multiple property settings**: For settings that control multiple styles or layout behaviors, directly assign Tailwind utility classes as the setting values:
```liquid
...
{% schema %}
{
  "settings": [{
    ...
    "label": "layout",
    "values": [
      { "value": "w-full max-w-none", "label": "t:options.full" },
      { "value": "max-w-7xl mx-auto px-4", "label": "t:options.narrow" }
    ]
  }]
}
{% endschema %}
